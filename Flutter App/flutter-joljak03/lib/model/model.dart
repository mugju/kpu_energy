...
// async: Future 문법을 가독성 있고, 간결하게 표현하기 위한 설정
Future<void> updateControlItems() async {
  ...
  // _jsonText에 sample_details.json을 저장
  final String _jsonText =
      await rootBundle.loadString('lib/sample_details.json');

  // SubItem(실제로 3단계까지 내려가나봄) 제어하기 위해 List 생성
  ...
  // _jsonText 저장한 json 내용 decode 해서 categoryList에 저장
  final List<dynamic> categoryList = json.decode(_jsonText) as List<dynamic>;
  for (int index = 0; index < categoryList.length; index++) {

    // 최종적으로 해당 데이터를 SampleBrowser 에서 관리할 수 있게 하는 SampleModel 클래스에 for문으로 저장
    ...
    if (SampleModel._categoryList[index].platformsToHide == null ||
      ...
      for (int i = 0;
        ...
        if (controlList[i].platformsToHide == null ||
          ...
          // 뒤에 따라오는 subItems 들도 아까 만들어 놨던 리스트들에 순차적으로 넣어준다.
