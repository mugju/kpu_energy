...
class ButtonZooming6_1 extends SampleView {
  /// Creates the chart with custom zooming buttons.
  const ButtonZooming6_1(Key key) : super(key: key);

  @override
  _ButtonZooming6_1State createState() => _ButtonZooming6_1State();
}

/// State class of the chart with custom zooming buttons.
class _ButtonZooming6_1State extends SampleViewState {
  _ButtonZooming6_1State();
  ...
  void _getSmartPlug() {
    ...
    db.getConnection().then((conn) {
      String sql =
          "select date_format(datetime, '%Y-%m-%d') DateTime, round(sum(amp), 2) amp from db.Smart_plug where id NOT IN (111) and id=6 and date_format(datetime, '%Y-%m-%d') is not null and datetime NOT IN ('2020-11-01 00:00:00') group by date_format(db.Smart_plug.datetime, '%Y-%m-%d')";
