...
class ButtonZooming2_2 extends SampleView {
  /// Creates the chart with custom zooming buttons.
  const ButtonZooming2_2(Key key) : super(key: key);

  @override
  _ButtonZooming2_2State createState() => _ButtonZooming2_2State();
}

/// State class of the chart with custom zooming buttons.
class _ButtonZooming2_2State extends SampleViewState {
  _ButtonZooming2_2State();
  ...
  void _getSmartPlug() {
    ...
    db.getConnection().then((conn) {
      String sql =
          "select date_format(datetime, '%Y-%m-%d %H:00') DateTime, round(sum(amp), 2) amp from db.Smart_plug where id NOT IN (111) and DateTime > ('2020-9-30 23:55:00')  and id=2 and date_format(datetime, '%Y-%m-%d %H:00') is not null and datetime NOT IN ('2020-11-01 00:00:00') group by date_format(db.Smart_plug.datetime, '%Y-%m-%d %H:00')";
