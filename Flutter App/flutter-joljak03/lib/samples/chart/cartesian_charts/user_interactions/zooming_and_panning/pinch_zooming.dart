...
class _DefaultPanningState extends SampleViewState {
    ...
    return StatefulBuilder(
      ...
      return ListView(
        ...
        children: <Widget>[
          Container(
            child: Row(
              // utf-8로 변경함/ 줌 모드를 x축 y축 x/y축으로 설정하는 부분
              children: <Widget>[
                Text('줌 모드 ',
                    style: TextStyle(
  ...
  SfCartesianChart _buildDefaultPanningChart() {
    return SfCartesianChart(
        key: chartKey,
        plotAreaBorderWidth: 0, // 차트 테두리 굵기
        primaryXAxis: DateTimeAxis(
            name: 'X-Axis',
            majorGridLines: const MajorGridLines(width: 0)), // 세로 구분 선 굵기
        primaryYAxis: NumericAxis(
            axisLine: const AxisLine(width: 0), // y 축 굵기
            anchorRangeToVisiblePoints: _enableAnchor,
            majorTickLines: const MajorTickLines(size: 0)), //왼쪽 틈 굵기
  ...
  List<AreaSeries<ChartSampleData, DateTime>> getDefaultPanningSeries() {
    ...
    return <AreaSeries<ChartSampleData, DateTime>>[
      AreaSeries<ChartSampleData, DateTime>(
          dataSource: getDateTimeData(), // getDateTimeData()의 데이터를 넣겠다.
          xValueMapper: (ChartSampleData sales, _) => sales.x as DateTime, // x 좌표는 DateTime으로 넣겠다.
