...
class AirConditioner2 extends SampleView {
  /// Creates the chart with custom zooming buttons.
  const AirConditioner2(Key key) : super(key: key);

  @override
  _AirConditioner2State createState() => _AirConditioner2State();
}

/// State class of the chart with custom zooming buttons.
class _AirConditioner2State extends SampleViewState {
  _AirConditioner2State();
  ...
  void _getSmartPlug() {
    ...
    db.getConnection().then((conn) {
      String sql =
          "select date_format(datetime, '%Y-%m-%d %H:00') DateTime, round(sum(amp), 3) amp from db.Airconditioner where date_format(datetime, '%Y-%m-%d %H:00') is not null and DateTime > ('2020-9-30 23:55:00') group by date_format(db.Airconditioner.datetime, '%Y-%m-%d %H:00')";
