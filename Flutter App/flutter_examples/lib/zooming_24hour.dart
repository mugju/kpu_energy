...
class ButtonZooming24hour extends SampleView {
  /// Creates the chart with custom zooming buttons.
  const ButtonZooming24hour(Key key) : super(key: key);

  @override
  _ButtonZooming24hourState createState() => _ButtonZooming24hourState();
}

/// State class of the chart with custom zooming buttons.
class _ButtonZooming24hourState extends SampleViewState {
  _ButtonZooming24hourState();
  ...
  void _getSmartPlug() {
    ...
    db.getConnection().then((conn) {
      String sql =
          "select date_format(datetime, '0000-00-00 %H:00') DateTime, round(sum(amp)/31, 3) amp from db.Smart_plug where id NOT IN (111) and date_format(datetime, '%Y-%m-%d %H:00') is not null and DateTime > ('2020-9-30 23:55:00') and datetime NOT IN ('2020-11-01 00:00:00') group by date_format(datetime, '0000-00-00 %H:00')";
